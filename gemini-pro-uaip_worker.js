...
// =================================================================
//                 UI/UX Generation Function (New)
// =================================================================
/**
 * Generates the VLESS configuration page with a modern UI/UX based on the Ukrainian theme.
 * @param {string} userID 
 * @param {string | null} hostName 
 * @returns {string}
 */
function getVlessConfig(userID, hostName) {
  // Generate various configuration links
  ...
  const workerNodes = `
    <div class="card">
        <h3>VLESS + WS (非TLS)</h3>
        <p>适合在域名被封锁时使用，无需TLS加密。</p>
        <div class="copy-item" data-copy="${wVlessws}">
            <pre><code>${wVlessws}</code></pre>
  ...
  const pageNodes = `
    <div class="card">
        <h3>VLESS + WS + TLS</h3>
        <p>标准TLS加密节点，安全稳定。推荐开启客户端分片(Fragment)功能以增强抗封锁性。</p>
        <div class="copy-item" data-copy="${pVlesswstls}">
            <pre><code>${pVlesswstls}</code></pre>
            <button class="copy-btn">复制</button>
        </div>
    </div>
  `;
  ...
  const allSubs = `
    <div class="card">
      <h3>通用订阅 (包含所有13个节点)</h3>
      <div class="copy-item" data-copy="${b64Links.all}">
          <span>Base64 分享链接 (可直接导入客户端)</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.ty}">
          <span>通用订阅链接 (TY)</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.cl}">
          <span>Clash-Meta 订阅</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.sb}">
          <span>Sing-Box 订阅</span>
          <button class="copy-btn">复制</button>
      </div>
    </div>
  `;
  ...
  const tlsSubs = `
    <div class="card">
      <h3>TLS 订阅 (仅包含6个TLS节点)</h3>
       <div class="copy-item" data-copy="${b64Links.tls}">
          <span>Base64 分享链接 (可直接导入客户端)</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.pty}">
          <span>通用订阅链接 (PTY)</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.pcl}">
          <span>Clash-Meta 订阅</span>
          <button class="copy-btn">复制</button>
      </div>
      <div class="copy-item" data-copy="${subLinks.psb}">
          <span>Sing-Box 订阅</span>
          <button class="copy-btn">复制</button>
      </div>
    </div>
  ...
  const manualConfig = `
    <div class="card-grid">
        <div class="card">
            <h4>非 TLS 连接参数</h4>
            <ul>
                <li><b>地址 (Address):</b> 优选IP / 域名</li>
                <li><b>端口 (Port):</b> 80, 8080, 8880, 2052, 2082, 2086, 2095</li>
                <li><b>用户ID (UUID):</b> ${userID}</li>
                <li><b>传输协议 (Network):</b> ws (websocket)</li>
                <li><b>伪装域名 (Host):</b> ${hostName}</li>
                <li><b>路径 (Path):</b> /?ed=2560</li>
                <li><b>传输安全 (TLS):</b> none</li>
            </ul>
        </div>
        <div class="card">
            <h4>TLS 连接参数</h4>
            <ul>
                <li><b>地址 (Address):</b> ${hostName}</li>
                <li><b>端口 (Port):</b> 443, 8443, 2053, 2083, 2087, 2096</li>
                <li><b>用户ID (UUID):</b> ${userID}</li>
                <li><b>传输协议 (Network):</b> ws (websocket)</li>
                <li><b>伪装域名 (Host):</b> ${hostName}</li>
                <li><b>路径 (Path):</b> /?ed=2560</li>
                <li><b>传输安全 (TLS):</b> tls</li>
                <li><b>SNI:</b> ${hostName}</li>
            </ul>
        </div>
    </div>
  `;
...
<head>
    ...
    <style>
        ...
        header h1 .yellow { color: var(--ukraine-yellow); }
        header p {
            color: #666;
            margin-top: 0.5rem;
        }
        /* New Statement Banner Style */
        .statement-banner {
        ...
        .copy-btn:hover { background-color: #004999; }
        ...
        ul li b { color: var(--heading-color); }
        footer {
            text-align: center;
            margin-top: 2rem;
            padding-top: 1rem;
...
<body>
    <div class="container">
        <header>
            <h1>VLESS <span class="yellow">配置中心</span></h1>
            <p>Cloudflare VLESS-WS 代理脚本 V25.5.4</p>
        </header>

        <!-- New Statement Banner -->
        <div class="statement-banner">
            <p>🇺🇦 我们坚定地与乌克兰站在一起，强烈谴责俄罗斯的非法侵略及其战争罪行。</p>
            <p style="font-size: 0.9em; opacity: 0.9;">We stand firmly with Ukraine and condemn Russia's illegal invasion and war crimes.</p>
        </div>

        <nav class="tabs">
            <button class="tab-btn active" data-tab="nodes">节点链接</button>
            <button class="tab-btn" data-tab="subs">订阅链接</button>
            <button class="tab-btn" data-tab="manual">手动配置</button>
        </nav>

        <main>
            <div id="nodes" class="tab-content active">
                ${isWorkersDev ? workerNodes : ''}
                ${pageNodes}
            </div>
            <div id="subs" class="tab-content">
                ${isWorkersDev ? allSubs : tlsSubs}
            </div>
