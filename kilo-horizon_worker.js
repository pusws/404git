...
.card .card-header{ background:transparent; border-bottom:1px solid #1f2937; }
.text-muted{ color:#9ca3af !important; }
.code-badge{ font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; padding:.35rem .5rem; border-radius:8px; background:#0f172a; border:1px solid #1e293b; color:#cbd5e1; }
.btn-primary{ background:#2563eb; border-color:#2563eb; }
.btn-outline-light{ border-color:#6b7280; color:#e5e7eb; }
.btn-outline-light:hover{ background:#1f2937; }
.small{ font-size:.875rem }
.copy-btn{ white-space:nowrap }
pre{ background:#0f172a; border:1px solid #1e293b; color:#cbd5e1; padding:.75rem; border-radius:12px; max-height:220px; overflow:auto; }
.badge-port{ background:#0b3b2e; color:#a7f3d0; border:1px solid #064e3b; }
section+section{ margin-top:var(--gap) }
.navbar{ background:#0f172a !important; border-bottom:1px solid #1f2937; }
...
<nav class="navbar navbar-dark sticky-top">
  <div class="container">
    ...
    <span class="small text-muted">UUID: ${userID.slice(0,8)}…</span>
  </div>
</nav>
...
`;
  // 卡片组件
  const card = (title, subtitle, content, actions='') => `
  ...
  // 节点卡片（单链接 + 复制）
  const singleLinkBlock = (label, link) => `
    <div class="d-flex flex-wrap align-items-center gap-2">
      <span class="code-badge flex-grow-1">${link}</span>
      <button class="btn btn-sm btn-outline-light copy-btn" onclick="copyToClipboard('${link}')">
  ...
  const helpBlock = `
    <div class="small text-muted">
      提示：如客户端支持分片(Fragment)可开启以降低域名阻断风险；订阅链接可直接复制或下载为文件导入客户端。
    </div>
    <hr/>
  `;
  if (hostName.includes("workers.dev")) {
    // Workers 域名：展示 ws(非TLS) 与 ws+tls 节点 + 全量/分类订阅
    const wsCard = card(
      'WS 节点（非 TLS）',
      '特性：关闭 TLS，适合规避域名阻断',
      helpBlock + singleLinkBlock('单节点', `${w\u0076\u006c\u0065\u0073\u0073ws}`),
      `<span class="badge badge-port rounded-pill px-3 py-2 small">端口: 80/8080/8880/2052/2082/2086/2095</span>`
    );
    const wstlsCard = card(
      'WS+TLS 节点',
      '特性：启用 TLS，建议客户端开启分片(Fragment)',
      singleLinkBlock('单节点', `${p\u0076\u006c\u0065\u0073\u0073wstls}`),
      `<span class="badge badge-port rounded-pill px-3 py-2 small">端口: 443/8443/2053/2083/2087/2096</span>`
    );
    const shareCard = card(
      '聚合通用分享（13 端口：7 非TLS + 6 TLS）',
      '',
      `
      <div class="d-flex flex-wrap align-items-center gap-2">
        <button class="btn btn-primary" onclick="copyToClipboard('${wk\u0076\u006c\u0065\u0073\u0073share}')">
          <i class="bi bi-clipboard-check"></i> 复制分享链接（Base64）
        </button>
        <button class="btn btn-outline-light" onclick="downloadText('share.txt', atob('${wk\u0076\u006c\u0065\u0073\u0073share}'))">
          <i class="bi bi-download"></i> 下载节点文本
        </button>
      </div>
      `,
      ``
    ...
    const subCard = card(
      '订阅链接',
      '注意：workers 域名作为订阅地址时，需要通过代理更新',
      `
      <div class="row g-2">
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">聚合通用订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${ty}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${ty}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('subscribe.txt','${ty}')">
        ...
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">Clash Meta 订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${cl}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${cl}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('clash.txt','${cl}')">
                <i class="bi bi-download"></i>
              </button>
            </div>
          </div>
        ...
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">Sing-box 订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${sb}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${sb}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('singbox.txt','${sb}')">
                <i class="bi bi-download"></i>
              </button>
            </div>
          </div>
...
`;
    ...
    // 自定义域 / Pages：展示 TLS 节点与 6 端口订阅
    const wstlsCard = card(
      'WS+TLS 节点',
      '特性：启用 TLS，建议客户端开启分片(Fragment)',
      helpBlock + singleLinkBlock('单节点', `${p\u0076\u006c\u0065\u0073\u0073wstls}`),
      `<span class="badge badge-port rounded-pill px-3 py-2 small">端口: 443/8443/2053/2083/2087/2096</span>`
    );
    const shareCard = card(
      '聚合通用分享（仅 6 个 TLS 端口）',
      ...
      `
      <div class="d-flex flex-wrap align-items-center gap-2">
        <button class="btn btn-primary" onclick="copyToClipboard('${pg\u0076\u006c\u0065\u0073\u0073share}')">
          <i class="bi bi-clipboard-check"></i> 复制分享链接（Base64）
        </button>
        <button class="btn btn-outline-light" onclick="downloadText('share_tls.txt', atob('${pg\u0076\u006c\u0065\u0073\u0073share}'))">
          <i class="bi bi-download"></i> 下载节点文本
        </button>
      </div>
      `,
      ...
      <div class="row g-2">
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">聚合通用订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${pty}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${pty}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('subscribe_tls.txt','${pty}')">
                <i class="bi bi-download"></i>
              </button>
            </div>
          </div>
        ...
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">Clash Meta 订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${pcl}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${pcl}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('clash_tls.txt','${pcl}')">
                <i class="bi bi-download"></i>
              </button>
            </div>
          </div>
        ...
        <div class="col-12 col-lg-4">
          <div class="d-flex flex-column gap-2">
            <div class="small text-muted">Sing-box 订阅</div>
            <div class="d-flex align-items-center gap-2">
              <span class="code-badge flex-grow-1">${psb}</span>
              <button class="btn btn-sm btn-outline-light" onclick="copyToClipboard('${psb}')">
                <i class="bi bi-clipboard-check"></i> 复制
              </button>
              <button class="btn btn-sm btn-outline-light" onclick="downloadText('singbox_tls.txt','${psb}')">
                <i class="bi bi-download"></i>
              </button>
            </div>
          </div>
