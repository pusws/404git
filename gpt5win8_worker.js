...
<style>
:root { --bg:#0f0f10; --text:#fff; --muted:#c8c8c8; --cobalt:#0078d7; --emerald:#107c10; --orange:#f7630c; --magenta:#d13438; --purple:#6b69d6; --teal:#009ca4; }
*{box-sizing:border-box}
html,body{height:100%}
body{margin:0;background:var(--bg);color:var(--text);font-family:-apple-system, Segoe UI, Roboto, Arial, "Microsoft YaHei", sans-serif}
.metro{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));grid-auto-rows:120px;gap:12px;padding:24px;max-width:1200px;margin:0 auto}
.tile{position:relative;display:flex;flex-direction:column;justify-content:flex-end;padding:16px;border-radius:4px;background:#2b2b2b;color:#fff;overflow:hidden;transition:transform .2s ease}
.tile:hover{transform:scale(1.02)}
.tile.cobalt{background:var(--cobalt)} .tile.emerald{background:var(--emerald)} .tile.orange{background:var(--orange)} .tile.magenta{background:var(--magenta)} .tile.purple{background:var(--purple)} .tile.teal{background:var(--teal)}
.tile.wide{grid-column:span 2}
.tile.tall{grid-row:span 2}
.tile.large{grid-column:span 2;grid-row:span 2}
.tile .icon{position:absolute;top:12px;left:12px;opacity:.9}
.tile .title{font-size:20px;line-height:1.2;margin:0 0 6px 0;font-weight:600}
.tile .desc{font-size:13px;color:rgba(255,255,255,.88)}
.tile .badge{position:absolute;top:10px;right:10px;background:rgba(255,255,255,.15);color:#fff;border:2px solid rgba(255,255,255,.65);border-radius:20px;padding:2px 8px;font-weight:700;font-size:12px;animation:pulse 2.8s ease-in-out infinite}
.tile .cta{align-self:flex-start;margin-top:10px;background:transparent;border:2px solid rgba(255,255,255,.85);color:#fff;padding:6px 10px;border-radius:3px;cursor:pointer;font-weight:600}
.tile .cta:hover{background:rgba(255,255,255,.12)}
.tile .mono{font-family:ui-monospace, SFMono-Regular, Consolas, "Liberation Mono", Menlo, monospace;font-size:12px;opacity:.95}
.limited{max-width:100%;word-break:break-all}
.marquee{position:absolute;inset:auto 0 0 0;height:28px;background:rgba(0,0,0,.12);display:flex;align-items:center;overflow:hidden;padding:0 8px}
.marquee span{white-space:nowrap;display:inline-block;padding-left:100%;animation:scroll 16s linear infinite}
.time{font-size:42px;font-weight:800;letter-spacing:.5px}
/* Flip live tile */
.tile.live.flip{perspective:800px}
.tile.live.flip .face,.tile.live.flip .back{position:absolute;inset:0;padding:16px;display:flex;flex-direction:column;justify-content:flex-end;backface-visibility:hidden;transform-style:preserve-3d}
.tile.live.flip .back{transform:rotateY(180deg)}
.tile.live.flip.flipped .face{transform:rotateY(180deg)}
.tile.live.flip.flipped .back{transform:rotateY(360deg)}
/* Ticker slide */
.tile.live.slide .ticker{overflow:hidden;will-change:transform}
.tile.live.slide .line{line-height:1.4}
@keyframes scroll{to{transform:translateX(-100%)}}
@keyframes pulse{0%,100%{opacity:.9}50%{opacity:.6}}
@media (prefers-reduced-motion:reduce){.tile,.badge,*{animation:none!important;transition:none!important}.marquee span{animation:none}}
...
function fallbackCopy(text){
  ...
  input.style.position='fixed';input.style.opacity=0;input.value=text;document.body.appendChild(input);input.select();document.execCommand('Copy');document.body.removeChild(input);alert('已复制到剪贴板');
}
function prefersReducedMotion(){return window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches}
function startFlip(){ if(prefersReducedMotion()) return; const flips=document.querySelectorAll('.tile.live.flip'); let i=0; flips.forEach(el=>{ setTimeout(()=>{ el.classList.toggle('flipped'); }, 900+i*300); setInterval(()=>{ el.classList.toggle('flipped'); }, 5200+(i++%3)*500); }); }
function startClock(){ const el=document.getElementById('clockTime'); if(!el) return; const upd=()=>{ const d=new Date(); const h=String(d.getHours()).padStart(2,'0'); const m=String(d.getMinutes()).padStart(2,'0'); el.textContent=h+':'+m; }; upd(); setInterval(upd, 30000); }
function startTicker(){ if(prefersReducedMotion()) return; document.querySelectorAll('[data-ticker]').forEach(t=>{ const lines=t.querySelectorAll('.line'); if(lines.length<=1) return; let idx=0; setInterval(()=>{ idx=(idx+1)%lines.length; t.style.transform='translateY(-'+(idx*1.4)+'em)'; }, 3500); }); }
window.addEventListener('load',()=>{ startFlip(); startClock(); startTicker(); });
</script>
</head>`;
...
<body>
  <div class="metro">
    <div class="tile large cobalt">
      <div class="icon">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="3" width="7" height="7" fill="white"/><rect x="14" y="3" width="7" height="7" fill="white"/><rect x="3" y="14" width="7" height="7" fill="white"/><rect x="14" y="14" width="7" height="7" fill="white"/></svg>
      </div>
      <h2 class="title">Workers VLESS 代理 V25.5.27</h2>
      <div class="desc">域名：${hostName}</div>
      <div class="mono">uuid: ${userID}</div>
      <div class="marquee"><span>${noteshow}</span></div>
    </div>

    <div class="tile emerald live flip">
      <span class="badge">WS</span>
      <div class="face">
        <h3 class="title">WS 节点</h3>
        <div class="desc">关闭 TLS，适合无视域名阻断</div>
        <button class="cta" onclick="copyToClipboard('${w\u0076\u006c\u0065\u0073\u0073ws}')">复制单节点链接</button>
      </div>
      <div class="back">
        <div class="title">节点链接</div>
        <div class="mono limited">${w\u0076\u006c\u0065\u0073\u0073ws}</div>
      </div>
    </div>

    <div class="tile cobalt live flip">
      <span class="badge">TLS</span>
      <div class="face">
        <h3 class="title">WS + TLS 节点</h3>
        <div class="desc">开启 TLS，客户端支持分片更佳</div>
        <button class="cta" onclick="copyToClipboard('${p\u0076\u006c\u0065\u0073\u0073wstls}')">复制单节点链接</button>
      </div>
      <div class="back">
        <div class="title">节点链接</div>
        <div class="mono limited">${p\u0076\u006c\u0065\u0073\u0073wstls}</div>
      </div>
    </div>

    <div class="tile teal wide">
      <div class="title">现在</div>
      <div class="time" id="clockTime">--:--</div>
      <div class="desc">本地时间</div>
    </div>

    <div class="tile orange live flip">
      <span class="badge">13</span>
      <div class="face">
        <h3 class="title">聚合通用分享</h3>
        <div class="desc">一键导入 (TLS+非TLS 共13端口)</div>
        <button class="cta" onclick="copyToClipboard('${wk\u0076\u006c\u0065\u0073\u0073share}')">复制分享链接</button>
      </div>
      <div class="back">
        <div class="title">包含端口</div>
        <div class="mono">80 / 8080 / 8880 / 2052 / 2082 / 2086 / 2095 + TLS 六端口</div>
      </div>
    </div>

    <div class="tile purple live flip">
      <div class="face">
        <h3 class="title">聚合订阅</h3>
        <div class="desc">通用客户端订阅链接</div>
        <button class="cta" onclick="copyToClipboard('${ty}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${ty}</div>
      </div>
    </div>

    <div class="tile magenta live flip">
      <div class="face">
        <h3 class="title">Clash-meta 订阅</h3>
        <div class="desc">Meta 内核专用</div>
        <button class="cta" onclick="copyToClipboard('${cl}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${cl}</div>
      </div>
    </div>

    <div class="tile teal live flip">
      <div class="face">
        <h3 class="title">Sing-box 订阅</h3>
        <div class="desc">Sing-box 客户端</div>
        <button class="cta" onclick="copyToClipboard('${sb}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${sb}</div>
        </div>
    </div>
</div>
</body>
...
<body>
  <div class="metro">
    <div class="tile large cobalt">
      ...
      <h2 class="title">VLESS 代理 V25.5.27</h2>
      <div class="desc">域名：${hostName}</div>
      <div class="mono">uuid: ${userID}</div>
      <div class="marquee"><span>${noteshow}</span></div>
    </div>
    ...
    <div class="tile orange live flip">
      ...
      <div class="face">
        <h3 class="title">聚合通用分享</h3>
        <div class="desc">仅 TLS 端口 (6 个)</div>
        <button class="cta" onclick="copyToClipboard('${pg\u0076\u006c\u0065\u0073\u0073share}')">复制分享链接</button>
      </div>
      <div class="back">
        <div class="title">包含端口</div>
        <div class="mono">443 / 8443 / 2053 / 2083 / 2087 / 2096</div>
      </div>
    </div>
    ...
    <div class="tile purple live flip">
      <div class="face">
        <h3 class="title">聚合订阅</h3>
        <div class="desc">通用客户端订阅链接</div>
        <button class="cta" onclick="copyToClipboard('${pty}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${pty}</div>
      </div>
    </div>
    ...
    <div class="tile magenta live flip">
      <div class="face">
        <h3 class="title">Clash-meta 订阅</h3>
        <div class="desc">Meta 内核专用</div>
        <button class="cta" onclick="copyToClipboard('${pcl}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${pcl}</div>
      </div>
    </div>
    ...
    <div class="tile teal live flip">
      <div class="face">
        <h3 class="title">Sing-box 订阅</h3>
        <div class="desc">Sing-box 客户端</div>
        <button class="cta" onclick="copyToClipboard('${psb}')">复制订阅</button>
      </div>
      <div class="back">
        <div class="title">订阅 URL</div>
        <div class="mono limited">${psb}</div>
        </div>
    </div>
</div>
</body>
