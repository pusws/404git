    ...
    async write(chunk) {
      ...
      if (webSocket.readyState === WS_READY_STATE_OPEN) {
        ...
        if (isvlessHeaderSent) {
          webSocket.send(await new Blob([udpSizeBuffer, dnsQueryResult]).arrayArrayBuffer());
        } else {
...
function getvlessConfig(userID, hostName) {
	...
	<body>
		<div class="container">
			<div class="tile" onclick="showModal('vless-ws-modal')">
				<div class="icon">🌐</div>
				<div class="title">VLESS+WS</div>
				<div class="subtitle">无TLS加密</div>
				<button class="copy-button" onclick="copyToClipboard(event, '${wvlessws}')">复制</button>
			</div>
			<div class="tile green" onclick="showModal('vless-wstls-modal')">
				<div class="icon">🔒</div>
				<div class="title">VLESS+WS+TLS</div>
				<div class="subtitle">TLS加密</div>
				<button class="copy-button" onclick="copyToClipboard(event, '${pvlesswstls}')">复制</button>
			</div>
			<div class="tile orange" onclick="showModal('subscription-modal')">
				<div class="icon">📋</div>
				<div class="title">订阅链接</div>
				<div class="subtitle">通用、Clash、Sing-box</div>
			</div>
			<div class="tile" onclick="showModal('notes-modal')" style="grid-column: span 2;">
				<div class="icon">📝</div>
				<div class="title">说明</div>
				<div class="subtitle">${note.split('\n')[0]}</div>
			</div>
		</div>

		<div id="vless-ws-modal" class="modal">
			<div class="modal-content">
				<span class="close" onclick="closeModal('vless-ws-modal')">&times;</span>
				<h2>VLESS+WS 配置</h2>
				<p><strong>特色:</strong> 关闭了TLS加密，无视域名阻断</p>
				<p><strong>链接:</strong> <code class="note">${wvlessws}</code></p>
				<button onclick="copyToClipboard(event, '${wvlessws}')">复制链接</button>
				<hr>
				<h4>客户端参数:</h4>
				<ul>
		...
		<div id="vless-wstls-modal" class="modal">
			<div class="modal-content">
				<span class="close" onclick="closeModal('vless-wstls-modal')">&times;</span>
				<h2>VLESS+WS+TLS 配置</h2>
				<p><strong>特色:</strong> 启用了TLS加密，如果客户端支持分片(Fragment)功能，建议开启，防止域名阻断</p>
				<p><strong>链接:</strong> <code class="note">${pvlesswstls}</code></p>
				<button onclick="copyToClipboard(event, '${pvlesswstls}')">复制链接</button>
				<hr>
				<h4>客户端参数:</h4>
				<ul>
					<li>客户端地址(address): 自定义的域名 或者 优选域名 或者 优选IP 或者 反代IP</li>
		...
		<div id="subscription-modal" class="modal">
			<div class="modal-content">
				...
				<hr>
				<h4>聚合通用分享链接 (可直接导入客户端):</h4>
				<p><code class="note">${wkvlessshare}</code></p>
				<button onclick="copyToClipboard(event, '${wkvlessshare}')">复制链接</button>
				<hr>
				<h4>聚合通用订阅链接:</h4>
				<p><code class="note">${ty}</code></p>
				<button onclick="copyToClipboard(event, '${ty}')">复制链接</button>
				<hr>
				<h4>Clash-meta订阅链接:</h4>
				<p><code class="note">${cl}</code></p>
				<button onclick="copyToClipboard(event, '${cl}')">复制链接</button>
				<hr>
				<h4>Sing-box订阅链接:</h4>
				<p><code class="note">${sb}</code></p>
				<button onclick="copyToClipboard(event, '${sb}')">复制链接</button>
			</div>
		</div>
