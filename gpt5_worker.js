...
<head>
  ...
  <style>
    :root { --brand-1: #6ea8fe; --brand-2: #a370f7; }
    html, body { height: 100%; }
    body {
      background: radial-gradient(1200px 600px at 10% 10%, rgba(110,168,254,.15), transparent),
                  radial-gradient(1000px 700px at 90% 20%, rgba(163,112,247,.12), transparent),
                  linear-gradient(180deg, #f8fafc, #f1f5f9);
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif;
      color: #1f2937;
    }
    .page-wrap { padding: 32px 12px; }
    .section-card {
    ...
    .table thead th { color: #475569; border-bottom: 1px solid rgba(15,23,42,.08); }
    .table tbody tr { border-top: 1px dashed rgba(15,23,42,.06); }
    .limited-width { max-width: 520px; overflow: auto; word-wrap: break-word; }
    .link-chip {
      display: inline-block; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace;
      font-size: 12px; padding: 8px 10px; border-radius: 10px; background: #0b1220; color: #cbd5e1;
      border: 1px solid rgba(148,163,184,.18); box-shadow: inset 0 1px rgba(255,255,255,.04);
      max-width: 520px; overflow: auto;
    }
    .btn-primary { background: linear-gradient(90deg, var(--brand-1), var(--brand-2)); border: 0; }
    .btn-primary:hover { filter: brightness(.95); }
    .badge-soft { background: rgba(110,168,254,.15); color: #1d4ed8; border: 1px solid rgba(110,168,254,.25); }
  </style>
</head>
<script>
  function showToast(message) {
    const area = document.getElementById('toastArea');
    if (!area || typeof bootstrap === 'undefined') { alert(message); return; }
...
<body class="page-wrap">
  ...
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-lg-10 section-card">
        <div class="d-flex align-items-center justify-content-between mb-2">
          <h1 class="h3 gradient-title mb-0">Cloudflare-workers/pages-\u0076\u006c\u0065\u0073\u0073代理脚本 V25.5.27</h1>
          <span class="badge rounded-pill badge-soft">UI Enhanced</span>
        </div>
        <p class="subtitle">${noteshow}</p>
      </div>

      <div class="col-lg-10 section-card">
        <h3 class="h5 mb-3">1：CF-workers-\u0076\u006c\u0065\u0073\u0073 + WS 节点</h3>
        <table class="table table-hover align-middle">
          <thead>
            <tr>
              <th style="width: 30%">节点特色</th>
              <th>单节点链接</th>
          ...
          <tbody>
            <tr>
              <td class="text-muted">关闭 TLS 加密，可缓解域名阻断</td>
              <td class="limited-width"><span class="link-chip">${w\u0076\u006c\u0065\u0073\u0073ws}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${w\u0076\u006c\u0065\u0073\u0073ws}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
          </tbody>
        </table>
        <div class="small text-muted">
          <div>客户端参数：</div>
          <ul class="mb-0">
            <li>地址：自定义域 / 优选域 / 优选IP / 反代IP</li>
            <li>端口：80、8080、8880、2052、2082、2086、2095</li>
            <li>UUID：${userID}</li>
            <li>传输：ws 或 websocket，Host：${hostName}，路径：/?ed=2560</li>
            <li>TLS：关闭</li>
          </ul>
        </div>
      </div>
      ...
      <div class="col-lg-10 section-card">
        <h3 class="h5 mb-3">2：CF-workers-\u0076\u006c\u0065\u0073\u0073 + WS + TLS 节点</h3>
        <table class="table table-hover align-middle">
          <thead>
            <tr>
              <th style="width: 30%">节点特色</th>
          ...
          <tbody>
            <tr>
              <td class="text-muted">启用 TLS。如客户端支持 Fragment 建议开启以防域名阻断</td>
              <td class="limited-width"><span class="link-chip">${p\u0076\u006c\u0065\u0073\u0073wstls}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${p\u0076\u006c\u0065\u0073\u0073wstls}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
          </tbody>
        </table>
        <div class="small text-muted">
          ...
          <ul class="mb-0">
            <li>地址：自定义域 / 优选域 / 优选IP / 反代IP</li>
            <li>端口：443、8443、2053、2083、2087、2096</li>
            <li>UUID：${userID}</li>
            <li>传输：ws 或 websocket，Host：${hostName}，路径：/?ed=2560</li>
            <li>TLS：开启，跳过证书验证：false</li>
          </ul>
        </div>
      </div>

      <div class="col-lg-10 section-card">
        <h3 class="h5 mb-3">3：聚合通用、Clash Meta、Sing-box 订阅</h3>
        <p class="small text-muted mb-3">注意：1) 每个订阅包含 TLS+非 TLS 共 13 个端口节点；2) 使用 workers.dev 域名作为订阅时需经代理更新；3) 若客户端不支持分片，TLS 节点不可用。</p>
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <button class="btn btn-primary btn-sm" onclick="copyToClipboard('${wk\u0076\u006c\u0065\u0073\u0073share}')"><i class="bi bi-clipboard-check"></i> 复制聚合通用分享</button>
        </div>
        <table class="table table-hover align-middle">
          <tbody>
            <tr>
              <th style="width: 220px">聚合通用订阅</th>
              <td class="limited-width"><span class="link-chip">${ty}</span></td>
              <td style="width: 120px"><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${ty}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
            <tr>
              <th>Clash-meta 订阅</th>
              <td class="limited-width"><span class="link-chip">${cl}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${cl}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
            <tr>
              <th>Sing-box 订阅</th>
              <td class="limited-width"><span class="link-chip">${sb}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${sb}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
          </tbody>
        </table>
      </div>
...
<body class="page-wrap">
  ...
  <div class="container">
    <div class="row justify-content-center">
      ...
      <div class="col-lg-10 section-card">
        <h3 class="h5 mb-3">1：CF-pages/workers/自定义域 - \u0076\u006c\u0065\u0073\u0073 + WS + TLS 节点</h3>
        <table class="table table-hover align-middle">
          <thead>
            <tr>
              <th style="width: 30%">节点特色</th>
          ...
          <tbody>
            <tr>
              <td class="text-muted">启用 TLS。如客户端支持分片(Fragment)可开启以防域名阻断</td>
              <td class="limited-width"><span class="link-chip">${p\u0076\u006c\u0065\u0073\u0073wstls}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${p\u0076\u006c\u0065\u0073\u0073wstls}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
          </tbody>
      ...
      <div class="col-lg-10 section-card">
        <h3 class="h5 mb-3">2：聚合通用、Clash Meta、Sing-box 订阅</h3>
        <p class="small text-muted mb-3">以下订阅仅包含 6 个 TLS 端口节点</p>
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <button class="btn btn-primary btn-sm" onclick="copyToClipboard('${pg\u0076\u006c\u0065\u0073\u0073share}')"><i class="bi bi-clipboard-check"></i> 复制聚合通用分享</button>
        </div>
        <table class="table table-hover align-middle">
          <tbody>
            <tr>
              <th style="width: 220px">聚合通用订阅</th>
              <td class="limited-width"><span class="link-chip">${pty}</span></td>
              <td style="width: 120px"><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${pty}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
            <tr>
              <th>Clash-meta 订阅</th>
              <td class="limited-width"><span class="link-chip">${pcl}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${pcl}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
            <tr>
              <th>Sing-box 订阅</th>
              <td class="limited-width"><span class="link-chip">${psb}</span></td>
              <td><button class="btn btn-primary btn-sm" onclick="copyToClipboard('${psb}')"><i class="bi bi-clipboard"></i> 复制</button></td>
            </tr>
          </tbody>
        </table>
      </div>
