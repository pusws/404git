...
<style>
body { font-family: 'Segoe UI', sans-serif; background-color: #0078D7; color: white; margin: 0; padding: 20px; }
.metro-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); grid-gap: 20px; }
.tile { background-color: #2C2C2C; padding: 20px; border-radius: 4px; text-align: center; cursor: pointer; transition: transform 0.2s; }
.tile:hover { transform: scale(1.05); }
.tile.blue { background-color: #0078D7; }
.tile.green { background-color: #107C10; }
.tile.orange { background-color: #D83B01; }
.tile h3 { margin: 10px 0; font-size: 18px; }
.tile p { font-size: 14px; }
.tile button { background-color: white; color: #0078D7; border: none; padding: 10px; border-radius: 4px; cursor: pointer; }
.limited-width { max-width: 180px; overflow: auto; word-wrap: break-word; margin: 0 auto; }
</style>
</head>
<script>
...
<div class="metro-container">
  <div class="tile blue">
    <h3>Cloudflare-workers/pages-vless代理脚本 V25.5.27</h3>
    <p>${noteshow}</p>
  </div>
  <div class="tile green">
    <h3>CF-workers-vless+ws节点</h3>
    <p class="limited-width">关闭了TLS加密，无视域名阻断</p>
    <p class="limited-width">${wvlessws}</p>
    <button onclick="copyToClipboard('${wvlessws}')">复制链接</button>
  </div>
  <div class="tile orange">
    <h3>CF-workers-vless+ws+tls节点</h3>
    <p class="limited-width">启用了TLS加密，如果客户端支持分片功能，建议开启</p>
    <p class="limited-width">${pvlesswstls}</p>
    <button onclick="copyToClipboard('${pvlesswstls}')">复制链接</button>
  </div>
  <div class="tile blue">
    <h3>聚合通用分享链接</h3>
    <button onclick="copyToClipboard('${wkvlessshare}')">复制链接</button>
  </div>
  <div class="tile green">
    <h3>聚合通用订阅链接</h3>
    <p class="limited-width">${ty}</p>
    <button onclick="copyToClipboard('${ty}')">复制链接</button>
  </div>
  <div class="tile orange">
    <h3>Clash-meta订阅链接</h3>
    <p class="limited-width">${cl}</p>
    <button onclick="copyToClipboard('${cl}')">复制链接</button>
  </div>
  <div class="tile blue">
    <h3>Sing-box订阅链接</h3>
    <p class="limited-width">${sb}</p>
    <button onclick="copyToClipboard('${sb}')">复制链接</button>
  </div>
  <div class="tile green">
    <h3>客户端参数 (ws)</h3>
    <p>地址: 自定义域名/优选IP</p>
...
<div class="metro-container">
  ...
  <div class="tile green">
    ...
    <p class="limited-width">启用了TLS加密，如果客户端支持分片功能，可开启</p>
    <p class="limited-width">${pvlesswstls}</p>
    <button onclick="copyToClipboard('${pvlesswstls}')">复制链接</button>
  </div>
  <div class="tile orange">
    <h3>聚合通用分享链接</h3>
    <button onclick="copyToClipboard('${pgvlessshare}')">复制链接</button>
  </div>
  <div class="tile blue">
    <h3>聚合通用订阅链接</h3>
    <p class="limited-width">${pty}</p>
    <button onclick="copyToClipboard('${pty}')">复制链接</button>
  </div>
  <div class="tile green">
    <h3>Clash-meta订阅链接</h3>
    <p class="limited-width">${pcl}</p>
    <button onclick="copyToClipboard('${pcl}')">复制链接</button>
  </div>
  <div class="tile orange">
    <h3>Sing-box订阅链接</h3>
    <p class="limited-width">${psb}</p>
    <button onclick="copyToClipboard('${psb}')">复制链接</button>
  </div>
  <div class="tile blue">
    <h3>客户端参数 (ws+tls)</h3>
    <p>地址: 自定义域名/优选IP</p>
